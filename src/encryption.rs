...
// pub type CipherKeyInput<'a> = Vec<&'a [u8]>;
// pub type CipherKeyInput = &[&[u8]];
// pub type CipherKey = [&[u8]];
// type CipherKeyInput<'a> = [&'a [u8]];
...
impl Cipher {
    /// # Panics
    /// If the slice `key_input` is empty
    ...
    pub fn new(key_input: &[&[u8]]) -> Self {
        ...
        // let key_bytes = hmac_sha256::Hash::hash(key);
        // let key: &Key<Aes256Gcm> = ;

        return Self {
            aes: Aes256Gcm::new((&key_bytes).into()),
        };
    }
...
mod test {
    ...
    fn different_key_cant_decrypt() {
        ...
        let wrong_cipher = Cipher::new(&[wrong_secret]);
        wrong_cipher
            .decrypt(&msg.ciphertext, &msg.nonce)
            .unwrap_err();
    }
